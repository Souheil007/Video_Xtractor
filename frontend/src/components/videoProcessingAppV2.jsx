...
function VideoProcessingApp() {
  ...
  const processVideo = async () => {
    if (isYoutube && youtubeUrl) {
      try {
        ...
        // Extract both programming language and code from the response
        const extractedData = response.data.map(entry => ({
            programming_language: entry.code_language, // Assuming code_language contains the programming language
            code: entry.code_result // Assuming code_result contains the code
        }));

        // Update your state with the extracted data
        setOutputText(extractedData); // This sets outputText to an array of objects with programming language and code
      } catch (error) {
  ...
  return (
    ...
    <div className="output-container">
        <h2>Output</h2>
        {/* Check if outputText has data */}
        {outputText.length > 0 ? (
            outputText.map((entry, index) => (
                <div key={index}>
                    <h4>Programming Language: {entry.programming_language}</h4>
                    <pre className="output-text">{entry.code}</pre> {/* Display code in a preformatted block */}
                </div>
            ))
        ) : (
            <p>No code extracted yet.</p> // Fallback message
        )}
    </div>
